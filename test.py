...
def universal_search(query, usersbox_api_key):
    """Универсальный поиск по доступным источникам (без Breachka и DepSearch)"""
    results = {}
...
def main():
    ...
    # API ключи (только UsersBox, так как Breachka и DepSearch исключены)
    usersbox_api_key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJjcmVhdGVkX2F0IjoxNzUxOTg0NTY1LCJhcHBfaWQiOjE3NTEzMTM5MTN9.pVrdFBE0wrutI9hvlZj74Od5I_nsEpLsNsLGsqQBYEk"
    ...
    while True:
        ...
        if choice == "7":
            # Универсальный поиск (без Breachka и DepSearch)
            results = universal_search(query, usersbox_api_key)
            for source, data in results.items():
                print_clean_data(data, source)
        else:
            # Поиск через UsersBox
